...
def render_mermaid_bar(impact: CostImpact) -> list[str]:
    """Render top cost deltas as a Mermaid xychart-beta bar."""
    from . import viz as viz_mod

    rows: list[tuple[str, float]] = []
    for r in impact.top_resources:
        name = str(r.get("name", "")).split("/")[-1] or "?"
        # Use absolute value so chart is meaningful even with mixed signs;
        # the table above already shows direction.
        rows.append((name, abs(float(r.get("monthly_diff", 0.0) or 0.0))))
    return viz_mod.mermaid_bar(
        f"Top cost deltas ({impact.currency}/mo)",
        rows,
        x_label=f"{impact.currency}/mo (abs)",
    )
