...
def test_duration_parsing():
    assert summarize_apply._duration_seconds("3s") == 3.0
    assert summarize_apply._duration_seconds("1m12s") == 72.0
    assert summarize_apply._duration_seconds("8.5s") == 8.5
    assert summarize_apply._duration_seconds("500ms") == 0.5
    assert summarize_apply._duration_seconds("") == 0.0
    assert summarize_apply._duration_seconds("2h3m4s") == 7384.0


def test_apply_with_timings_renders_gantt():
    log = Path(__file__).parent / "fixtures" / "apply_with_timings.log"
    text = log.read_text(encoding="utf-8")
    r = summarize_apply.parse_apply_log(text)
    assert r.succeeded
    md = summarize_apply.render_markdown(r)
    assert "## Apply dashboard" in md
    assert "## Slowest resources" in md
    assert "```mermaid" in md
    assert "gantt" in md
    # vm took the longest in fixture
    assert "azurerm_linux_virtual_machine.vm" in md
